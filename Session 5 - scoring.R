...
sub<-sample(1:nrow(data),round(nrow(data)*0.8))
length(sub)
# Show data
data_train<-data[sub,]
data_test<-data[-sub,]
...
data_test$probs  = predict(object = prob.model, newdata = data_test, type = "probs")
data_test$amount = exp(predict(object = amount.model, newdata = data_test))
data_test$score  = data_test$probs * data_test$amount
print(head(data_test))
data_test[is.na(data_test)] <- 0
data_test$diff=data_test$score-data_test$targetamount
sum(abs(data_test$diff))
...
z = which(out$score > 5)
x=subset(out,out$score > 5)
print(head(x))
out$diff=out$score-data_test$targetamount
sum(abs(data_test$diff))
